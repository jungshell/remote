...
import { submitSurveyResponseToSupabase } from "@/lib/supabase/responses";
...
export function SurveyForm({ survey }: SurveyFormProps) {
  ...
  async function handleVerify() {
    ...
    if (saved) {
      ...
      try {
        const response = await fetch(
          `/api/survey-responses/lookup?survey_id=${encodeURIComponent(survey.id)}&phone_last4=${phoneLast4}`,
        );
        ...
        if (data.ok && data.exists && Array.isArray(data.answers)) {
    ...
    setMessage(loaded ? "기존 응답을 불러왔습니다. 수정 후 다시 제출할 수 있습니다." : "");
    setStep("questions");
  ...
  return (
    <section className="mx-auto max-w-2xl">
      ...
      <div className="panel animate-enter overflow-hidden">
        <div className="border-b border-[var(--hairline)] p-6">
          <p className="label-machined text-[var(--text-muted)]">
            {survey.year} · {survey.division} · {survey.round}회차
          </p>
          <h1 className="mt-4 text-3xl font-black uppercase leading-tight">{survey.title}</h1>
        ...
        {step === "intro" ? (
          <div className="p-6">
            ...
            <button
              type="button"
              className="focus-ring label-machined mt-8 w-full border border-white px-6 py-4 transition-colors hover:bg-white hover:text-black"
        ...
        {step === "verify" ? (
          <div className="p-6">
            ...
            <input
              ...
              className="focus-ring mt-6 h-14 w-full border border-[var(--hairline)] bg-[var(--surface-soft)] px-4 text-center text-2xl font-black tracking-[0.35em] text-white"
              placeholder="0000"
            />
            ...
            <button
              ...
              className="focus-ring label-machined mt-8 w-full border border-white px-6 py-4 transition-colors hover:bg-white hover:text-black disabled:opacity-50"
    ...
    <fieldset className="border-b border-[var(--hairline)] pb-8 last:border-b-0">
      ...
      {question.scale === "text" ? (
        <textarea
          ...
          className="focus-ring mt-5 min-h-36 w-full border border-[var(--hairline)] bg-[var(--surface-soft)] p-4 leading-7 text-white"
