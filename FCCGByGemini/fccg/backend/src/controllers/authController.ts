...
export const checkMemberStatusRules = async (options?: { dryRun?: boolean }) => {
  ...
  const changes: StatusChangeRecord[] = [];
  const now = new Date();

  try {
    console.log(`🔍 회원 상태 자동 변경 규칙 체크 시작${dryRun ? ' (dryRun)' : ''}`);

    const allMembers = await prisma.user.findMany({
      where: {
        status: { in: ['ACTIVE', 'INACTIVE'] },
        role: { in: ['MEMBER'] }, // 관리자·슈퍼관리자는 자동 제재 제외
      },
      select: {
        id: true,
        name: true,
    ...
    const analysisStart =
      MEMBER_STATUS_RULES_START_AT > threeMonthsAgo
        ? MEMBER_STATUS_RULES_START_AT
        : threeMonthsAgo;

    // 완료된 투표 세션을 한 번만 조회 (회원별 참여 여부는 votes에서 확인)
    const completedSessions = await prisma.voteSession.findMany({
      where: {
        isCompleted: true,
        // 기준일 이후 끝난 세션만 새 규칙의 모수로 사용
        endTime: { gte: analysisStart },
      },
      select: {
        id: true,
        weekStartDate: true,
    ...
    const realGames = pastGames.filter((g) => {
      if (g.autoGenerated && (g.time === '미정' || g.location === '미정' || !g.time)) {
        return false;
      }
      return true;
    });
    ...
    for (const member of allMembers) {
      const voteStatus = evaluateVoteParticipation(member, completedSessions, now);
      const gameStatus = evaluateGameParticipation(
        member,
        realGames,
        now,
        MEMBER_STATUS_RULES_START_AT
      ...
      if (dryRun) {
        console.log(
          `🧪 [dryRun] ${member.name}(${member.id}): ${member.status} → ${newStatus} (${reason})`
        );
        continue;
      }
      ...
      console.log(
        `👤 회원 ${member.name}(${member.id}) 상태 변경: ${member.status} → ${newStatus} (${reason})`
      );
    ...
    console.log(
      `✅ 회원 상태 체크 완료 — 대상 ${allMembers.length}명, 변경 ${changes.length}건${dryRun ? ' (dryRun)' : ''}`
    );

    return {
      success: true,
      dryRun,
      rulesStartAt: MEMBER_STATUS_RULES_START_AT.toISOString(),
