...
function buildProcessedVoteSession(filteredActiveSession: any, allMembers: any[] = []): any | null {
  ...
  const participants = rawVotes.map((vote: any) => {
    ...
    const codes = rawDays
      .map((d) => voteDayToMonFriAbsentKeyForSession(d, weekStartForSession))
  ...
  participants.forEach((participant: any) => {
    ...
    selectedDaysArray.forEach((day: string) => {
      const k = voteDayToMonFriAbsentKeyForSession(day, weekStartForSession);
      if (k === '불참') {
        results['불참'].count++;
      ...
      } else if (k && results[k as keyof typeof results]) {
        results[k as keyof typeof results].count++;
        results[k as keyof typeof results].participants.push({
          userId: participant.userId,
          userName: participant.userName,
          votedAt: participant.votedAt
        });
...
// 메일 발송은 서버의 Gmail 앱 비밀번호(SMTP) 방식만 사용한다.
// 과거 OAuth 콜백은 state 검증 없이 refresh token을 화면에 노출하므로 폐기한다.
  ...
  try {
    ...
    try {
      ...
      if (currentUser.status !== 'ACTIVE') {
        return res.status(403).json({
          error: '현재 이용할 수 없는 계정은 투표할 수 없습니다. 관리자에게 확인 바랍니다.',
          memberStatus: currentUser.status
        });
      }
  ...
  try {
    ...
    try {
      ...
      if (time && location && eventType && 
        ...
        if (sendConfirmationNotification === true) {
          sendGameConfirmationNotification(updatedGame).catch(err =>
            console.error('❌ 게임 확정 알림 백그라운드 발송 실패:', err)
          );
        }
      }
    ...
    const userId = (req as any).user?.userId;
      ...
      const game = await prisma.game.update({
        ...
        data: {
          ...
          memberNames: JSON.stringify(namesArray || []),
          selectedMembers: JSON.stringify(selectedArray || []),
          autoGenerated: isConfirmedGame ? false : existingGame.autoGenerated,
          confirmed: isConfirmedGame || existingGame.confirmed,
          updatedAt: new Date()
        },
        include: {
          attendances: {
      ...
      // 자동생성된 게임이 확정된 경우 처리
      let emailNotification: any = null;
      if (existingGame?.autoGenerated && isConfirmedGame) {
        ...
        // 게임 확정 알림 발송
        emailNotification = await sendGameConfirmationNotification(game);
      }

      res.json({
        message: '경기가 수정되었습니다.',
        game,
        autoGeneratedGamesDeleted: existingGame?.autoGenerated && isConfirmedGame,
        emailNotification
      });
...
router.post('/votes/aggregate/save', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    // 최신 투표 결과로 재생성
    const { counts, participantsByDay } = aggregateVotesByWeekday(session.votes, weekStart);
...
router.post('/send-vote-notification-test', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    console.log(`✅ 메일 전송 연결 성공 (${mailConfig.mode})`);
