...
/** 경기 목록 표시용 날짜 파싱(yyyy.mm.dd / ISO 등) — 정렬 실패 시 0 */
function parseGameDateMs(game: { date?: string }): number {
  if (!game?.date) return 0;
  const s = String(game.date);
...
export default function GameManagement({ games, onGamesChange, userList, onGameDataChanged }: GameManagementProps) {
  ...
  const filteredGames = React.useMemo(() => {
    ...
    */
    ...
    // 최신 경기일이 위로 오도록 정렬
    result.sort((a, b) => parseGameDateMs(b) - parseGameDateMs(a));
  ...
  const handleEditGame = (game: Game) => {
    ...
    // 날짜를 YYYY-MM-DD 형식으로 변환 (KST 로컬 날짜 기준)
    const gameDate = new Date(game.date);
    const _fy = gameDate.getFullYear();
    const _fm = String(gameDate.getMonth() + 1).padStart(2, '0');
    const _fd = String(gameDate.getDate()).padStart(2, '0');
    const formattedDate = `${_fy}-${_fm}-${_fd}`;
    ...
    setFormData({
      ...
      locationLatitude: game.locationLatitude ?? null,
      locationLongitude: game.locationLongitude ?? null,
  ...
  const selectLocation = (place: any) => {
    ...
    setFormData(prev => ({
      ...
      locationLatitude: Number(place.y) || null,
      locationLongitude: Number(place.x) || null,
      ...
      <VStack spacing={5} align="stretch">
        <Flex justify="space-between" align="flex-end" wrap="wrap" gap={3}>
          ...
          <Button 
            ...
            _hover={{ bg: "#003d7a", transform: 'scale(1.03)' }}
              ...
              <Table variant="simple">
                ...
                <Tbody>
                  {filteredGames.map((game) => (
                    <Tr
                      key={game.id}
                      bg={game.autoGenerated ? "blue.50" : "white"}
                      transition="background 0.2s ease, box-shadow 0.2s ease"
                      _hover={{
                        boxShadow: 'inset 0 0 0 1px rgba(0, 78, 168, 0.2)',
                        bg: game.autoGenerated ? 'blue.100' : 'gray.50',
                      }}
                    >
            ...
            <VStack spacing={2}>
              ...
              <FormControl>
                <HStack justify="space-between" align="center" mb={2}>
                  <FormLabel mb={0}>👥 확정 참가자 관리</FormLabel>
                  <Box ml={4}>
                ...
                <HStack spacing={4} align="start">
                  {/* 왼쪽: 회원 선택 */}
                  <Box flex={1}>
                    <Text fontSize="sm" color="gray.600" mb={1} fontWeight="bold">확정 참가 회원</Text>
                    <Text fontSize="xs" color="gray.500" mb={2}>투표 결과와 별개로 이 명단이 확정 일정과 메인 화면에 표시됩니다.</Text>
                    <Box>
                      {userList.map((user) => {
